...
describe("ChatPanel", () => {
  ...
  it("renders switch to form link", async () => {
    render(<ChatPanel {...defaultProps} />);

    const switchLink = screen.getByText(/switch to manual form/i);
    expect(switchLink).toBeInTheDocument();

    await userEvent.setup().click(switchLink);
    expect(defaultProps.onSwitchToForm).toHaveBeenCalled();
  });
